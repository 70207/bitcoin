...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    socklen_t len = sizeof(__sockaddr);
    pfrom->addr.GetSockAddr((struct sockaddr*)&__sockaddr, &len);
    const char* __ip = inet_ntoa(__sockaddr.sin_addr);
    int __port = ntohs(__sockaddr.sin_port);

    LogPrintX("pre recv command:%s, from ip:port, %s:%d\n", strCommand, __ip, __port);
    ...
    else if (strCommand == NetMsgType::ADDR)
    {
        ...
        LogPrintX("recv addr commmand:%s:%d\n", __ip, __port);
    ...
    else if (strCommand == NetMsgType::GETADDR)
    {
        ...
        // the getaddr message mitigates the attack.
         ...
         LogPrintX("recv getaddr commmand:%s:%d\n", __ip, __port);
    ...
    {
         LogPrintX("recv ping commmand:%s:%d\n", __ip, __port);
    ...
    {
        LogPrintX("recv pong commmand:%s:%d\n", __ip, __port);
